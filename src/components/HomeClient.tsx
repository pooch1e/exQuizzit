...
export default function HomeClient() {
  ...
  const fetchUserData = async (username: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const user = users.find((u: { userName: string; userId: number; quizzBuckTotal: number }) => u.userName === username);
        if (user) {
  ...
  const updateUserQuizzBucks = async (newAmount: number) => {
    if (!userId) return;
  ...
  const handleLootboxSpin = () => {
    // Make it free for now - always return true
    return true;
  };

  const handleThemeWin = (theme: Theme) => {
